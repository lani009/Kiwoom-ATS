...
from ats.StockException import NoSuchStockCodeError, NoSuchStockPositionError
...
class KiwoomDAO:
    ...
    def get_stock_name(self, stock_code: str) -> str:
        ...
        name = self.kiwoom_instance.dynamicCall("GetMasterCodeName(QString)", stock_code)
    ...
    def get_available_balance(self, acc_no: str) -> int:
        ...
        balance = int(
            self.__get_tr_data(
                {"계좌번호": acc_no, "비밀번호": "", "상장폐지조회구분": "1", "비밀번호입력매체구분": "00"},
                "주식 잔고 요청",
                "OPW00004",
                "0",
                "5000",
    ...
    def get_current_price(self, stock_code: str) -> int:
        if not self.__current_price_map.__contains__(stock_code):
            ...
            current_price: str = self.__get_tr_data(
                ...
                "OPT10003",
                "0",
                self.__generate_scr_no(stock_code),
                ["현재가"],
                [],
                cnt=1,
            )["single_data"]["현재가"]
    ...
    def __get_tr_data(
        ...
        perv_next: str,
        scr_no: str,
        rq_single_data: List[str],
        rq_multi_data: List[str],
        cnt=0,
    ):
    ...
    def __set_input_values(self, input_value: Dict[str, str]):
        ...
        for k, v in input_value.items():
            self.kiwoom_instance.dynamicCall("SetInputValue(QString, QString)", k, v)
    ...
    def __on_receive_tr_data(self, scr_no, rq_name, tr_code, prev_next):
        ...
        if self.__tr_data_cnt_limit == 0:
            n_record = self.kiwoom_instance.dynamicCall("GetRepeatCnt(QString, QString)", tr_code, rq_name)
        else:
            n_record = min(
                self.kiwoom_instance.dynamicCall("GetRepeatCnt(QString, QString)", tr_code, rq_name),
                self.__tr_data_cnt_limit,
            )
        ...
        for s_data in self.__tr_rq_single_data:
            self.__tr_data_temp["single_data"][s_data] = self.kiwoom_instance.dynamicCall(
                "GetCommData(QString, QString, int, QString)", tr_code, rq_name, 0, s_data
            ).strip()
        ...
        for i in range(n_record):
            ...
            for m_data in self.__tr_rq_multi_data:
                m_data_dict_temp[m_data] = self.kiwoom_instance.dynamicCall(
                    "GetCommData(QString, QString, int, QString)", tr_code, rq_name, i, m_data
                ).strip()
    ...
    def __on_receive_real_data(self, stock_code, real_type, real_data):
        ...
        if real_type == "주식체결":
            self.__current_price_map[stock_code] = abs(
                int(self.kiwoom_instance.dynamicCall("GetCommRealData(QString, int)", stock_code, 10))
            )
        elif real_type == "장시작시간":
            self.__market_status = int(
                self.kiwoom_instance.dynamicCall("GetCommRealData(QString, int)", stock_code, 215)
            )
    ...
    def __register_all_slots(self):
        self.kiwoom_instance.OnEventConnect.connect(self.__on_event_connect_slot)
        self.kiwoom_instance.OnReceiveRealData.connect(self.__on_receive_real_data)
        ...
        self.kiwoom_instance.OnReceiveChejanData.connect(self.__on_receive_chejan_data)
